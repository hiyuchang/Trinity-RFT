...
$$\mathcal{J}_{\text{Mix}}(\theta) = \mathcal{J}_{\text{GRPO}}(\theta) + \mu \cdot \frac{1}{B'} \sum_{b=1}^{B'} \left[\frac{1}{T'_b} \sum_{t=1}^{T'_b} \log \pi_\theta(o'_{b,t} \mid q'_b, o'_{b,<t}) \right].$$
