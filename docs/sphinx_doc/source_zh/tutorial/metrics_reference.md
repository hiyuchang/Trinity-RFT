...
- 从*Experiences*到*Run 级别*（仅对 `can_repeat=False` 的 workflows 有效）：在 `calculate_run_level_metrics` 函数中，在每次 rollout 生成的所有 experiences 中求平均得到 run 级别指标。这是为了适应通用多轮场景，其中单个 rollout 可能产生多个 experiences。
